...
* [`src/tools_methods_front/`](src/tools_methods_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/tools_methods_front/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `tools-methods-front` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tools_methods_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tools-methods-front.example
  (:require [tools-methods-front.config :as config])
